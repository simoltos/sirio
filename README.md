...
Many coding agents are large, layered stacks that are hard to understand end to end. ds4's agent layer stood out for the opposite reason: it is compact enough to understand and build upon.

Sirio keeps that shape while moving two boundaries. Inference goes to model provider APIs/OAuth, so one small program can drive different models without embedding an inference engine. Tool execution goes into a short-lived Podman container, so file, shell and web tools run off the host: an agent that runs arbitrary commands and edits gets an ephemeral, disposable environment. The goal is not a general agent framework: it is a small C program that stays easy to run, inspect and change.

Sirio is also a personal project — a way to understand how a coding agent actually works (the agent loop, tool protocols, provider streaming) by building and using one.

## How it works

Compared to ds4, Sirio changes where two things happen: inference is delegated to model provider APIs, while file, shell and web tool execution moves from the host into a container. Subagents remain host-side processes that launch their own containers. The agent loop itself keeps ds4's shape.

* **Podman is the tool boundary.** File edits, shell commands and web browsing execute inside a short-lived container started from the repository's `cma` image. The container is created with `--rm`, runs with `--userns=keep-id`, bind-mounts the selected workspace read-write at `/workspace`, and keeps outbound network access for the web and shell tools. The host talks to a tool runner inside the container over a small JSON protocol on pipes.
* **The `subagent` tool is the exception.** It starts a new Sirio process on the host, which then runs its own container for its tools.
...
* The selected workspace is bind-mounted read-write into the container, so tools can modify or delete anything in it.
* Tools keep outbound network access and can reach anything the container can reach.
* Credentials stay on the host: `auth.json` is never mounted into the container or passed to the tool runner.
* Prompts, workspace content and tool output may be sent to the configured model provider.
* Podman isolation is not an absolute sandbox: a hostile or misled agent can still alter the workspace and use the network.
...
The `subagent` tool can select any model in the compiled catalog, including models outside the interface set, when credentials for the corresponding provider are available. Each subagent run is a separate provider session and can add API costs on top of the main session. Subagent model restrictions and confirmation before delegation are not currently implemented.
...
**Migrating from `models.json`.** Earlier versions kept the interface set in `~/.sirio/models.json`; that file is now ignored, and the set is rebuilt with:

```sh
./sirio catalog --default provider/model[,provider/model...]
```
...
* Session history displayed by `sessions --resume`, `/switch`, or `/history` is not yet rendered exactly like live output; some formatting may appear as plain text.
